...
/**
 * @brief 统一轨迹规划入口节点，支持 MoveJ / MoveL / MoveC / JointConstrained
 * 四种规划策略
...
public:
	TrajectoryPlanningNode() : Node("trajectory_planning_node") {
		...
		sub_movej_ = this->create_subscription<sensor_msgs::msg::JointState>(
		    ...
		    std::bind(&TrajectoryPlanningNode::moveJCallback, this,
		              std::placeholders::_1));
		sub_movel_ = this->create_subscription<geometry_msgs::msg::Pose>(
		    "movel_goals_internal", qos,
		    std::bind(&TrajectoryPlanningNode::moveLCallback, this,
		              std::placeholders::_1));
		sub_movec_ =
		    this->create_subscription<robot_interfaces::msg::MoveCRequest>(
		        "movec_goals_internal", qos,
		        std::bind(&TrajectoryPlanningNode::moveCCallback, this,
		                  std::placeholders::_1));
		sub_joint_constrained_ = this->create_subscription<
		    robot_interfaces::msg::JointConstrainedRequest>(
		    "joint_constrained_goals_internal", qos,
		    std::bind(&TrajectoryPlanningNode::jointConstrainedCallback, this,
		              std::placeholders::_1));
	...
	void initializeComponents() {
		if (!motion_planning_service_) {
			...
			auto movel_strategy =
			    std::make_shared<MoveLPlanningStrategy>(*moveit_adapter_);
			auto movec_strategy =
			    std::make_shared<MoveCPlanningStrategy>(*moveit_adapter_);
			auto joint_constrained_strategy =
...
private:
	...
	void moveLCallback(const geometry_msgs::msg::Pose::SharedPtr msg) {
		...
		if (result.success) {
			...
			RCLCPP_ERROR(this->get_logger(), "MoveL 规划失败：%s",
			             result.error_message.c_str());
		}
	}
	...
	// 应用服务层
	// std::shared_ptr<trajectory_planning::application::services::MotionPlanningService>
	// motion_planning_service_;
	// std::shared_ptr<trajectory_planning::application::services::TrajectoryExecutionService>
	// trajectory_execution_service_;
	...
	rclcpp::Subscription<robot_interfaces::msg::MoveCRequest>::SharedPtr
	    sub_movec_;
	rclcpp::Subscription<robot_interfaces::msg::JointConstrainedRequest>::
	    SharedPtr sub_joint_constrained_;
};
