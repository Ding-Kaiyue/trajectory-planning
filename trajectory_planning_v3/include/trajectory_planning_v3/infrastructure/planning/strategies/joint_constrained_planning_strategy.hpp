...
public:
	...
	struct FixedJointConstraint : public JointConstraint {
		...
		FixedJointConstraint(int index, double value, double weight = 1.0,
		                     bool is_hard = true) {
			joint_index = index;
			fixed_value = value;
			this->weight = weight;
	...
	struct RangeJointConstraint : public JointConstraint {
		...
		RangeJointConstraint(int index, double min_val, double max_val,
		                     double weight = 1.0, bool is_hard = true) {
			joint_index = index;
			min_value = min_val;
			max_value = max_val;
			this->weight = weight;
	 ...
	 */
	explicit JointConstrainedPlanningStrategy(
	 ...
	 * @return 约束是否可行
	 */
	bool validateAllConstraints(const std::string& arm_type,
...
private:
	...
	// 验证参数
	static constexpr double DEFAULT_POSITION_TOLERANCE =
	    1e-3;  // 1mm for positions (more reasonable than 1e-6)
	static constexpr double DEFAULT_VELOCITY_TOLERANCE =
	    1e-2;  // 0.01 rad/s for velocities
	static constexpr double DEFAULT_ACCELERATION_TOLERANCE =
	    1e-1;  // 0.1 rad/s² for accelerations
	 ...
	 * @return 是否满足约束
	 */
	bool validateTrajectoryConstraints(
	 ...
	 */
	std::optional<std::vector<double>> generateSeedConfiguration(
