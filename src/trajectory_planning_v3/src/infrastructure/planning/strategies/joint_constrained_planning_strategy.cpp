...
void JointConstrainedPlanningStrategy::addConstraint(std::shared_ptr<JointConstraint> constraint, bool validate) {
    if (validate && constraint) {
        auto result = validateSingleConstraint(constraint, 1); // INFO/WARN level logging
        ...
        if (result == ConstraintValidationResult::RANGE_NO_INTERSECTION ||
            ...
            RCLCPP_WARN(rclcpp::get_logger("JointConstrainedPlanningStrategy"),
                       "Adding conflicting constraint anyway, but this will cause planning failures!");
        }
    }
...
bool JointConstrainedPlanningStrategy::validateConstraints(const std::vector<double>& joint_values) const {
    for (const auto& constraint : constraints_) {
        if (constraint->joint_index >= static_cast<int>(joint_values.size())) {
            continue; // 跳过超出范围的约束
        }

        if (auto fixed = std::dynamic_pointer_cast<FixedJointConstraint>(constraint)) {
            double error = std::abs(joint_values[fixed->joint_index] - fixed->fixed_value);
            if (error > DEFAULT_POSITION_TOLERANCE) {
                return false;
            }
        } else if (auto range = std::dynamic_pointer_cast<RangeJointConstraint>(constraint)) {
...
bool JointConstrainedPlanningStrategy::validateAllConstraints(const std::string& arm_type, int log_level) const {
    ...
    // 2. 验证同一关节的多个约束之间的兼容性
    std::map<int, std::vector<std::shared_ptr<JointConstraint>>> constraints_by_joint;

    // 按关节分组约束
    for (const auto& constraint : constraints_) {
        if (constraint->joint_index >= 0 && constraint->joint_index < static_cast<int>(joint_limits.size())) {
    ...
    for (const auto& [joint_index, joint_constraints] : constraints_by_joint) {
        ...
        for (const auto& constraint : joint_constraints) {
            ...
            if (!range.has_value()) {
                ...
                if (log_level >= 2) {
                    RCLCPP_ERROR(rclcpp::get_logger("ConstrainedPlanningStrategy"),
                                "CONSTRAINT CONFLICT: Joint %d has conflicting constraints", joint_index);
                }
                all_feasible = false;
                break;
            }
            ...
            if (effective_min > effective_max) {
                ...
                if (log_level >= 2) {
                    RCLCPP_ERROR(rclcpp::get_logger("ConstrainedPlanningStrategy"),
                                "CONSTRAINT CONFLICT: Joint %d multiple constraints have no intersection! "
        ...
        if (all_feasible && log_level >= 1 && joint_constraints.size() > 1) {
            RCLCPP_INFO(rclcpp::get_logger("ConstrainedPlanningStrategy"),
                       "Joint %d has %zu constraints, effective range: [%.3f, %.3f]",
                       joint_index, joint_constraints.size(), effective_min, effective_max);
        }
    }
...
JointConstrainedPlanningStrategy::validateSingleConstraint(
    ...
    // 检查不同类型的约束
    if (auto range = std::dynamic_pointer_cast<RangeJointConstraint>(constraint)) {
        double min_val = std::max(range->min_value, limit.first);
        double max_val = std::min(range->max_value, limit.second);

        if (min_val > max_val) {
            if (log_level >= 2) {
                RCLCPP_ERROR(rclcpp::get_logger("JointConstrainedPlanningStrategy"),
                            "CONSTRAINT CONFLICT: Joint %d range [%.3f, %.3f] does not intersect with joint limits [%.3f, %.3f]",
                            constraint->joint_index, range->min_value, range->max_value, limit.first, limit.second);
            }
            return ConstraintValidationResult::RANGE_NO_INTERSECTION;
        } else {
            if (log_level >= 1) {
                RCLCPP_INFO(rclcpp::get_logger("JointConstrainedPlanningStrategy"),
                           "Added range constraint: joint%d ∈ [%.3f, %.3f] (effective: [%.3f, %.3f])",
                           constraint->joint_index, range->min_value, range->max_value, min_val, max_val);
            }
        }
    } else if (auto fixed = std::dynamic_pointer_cast<FixedJointConstraint>(constraint)) {
        if (fixed->fixed_value < limit.first || fixed->fixed_value > limit.second) {
            if (log_level >= 2) {
                RCLCPP_ERROR(rclcpp::get_logger("JointConstrainedPlanningStrategy"),
                            "CONSTRAINT CONFLICT: Joint %d fixed value %.3f outside joint limits [%.3f, %.3f]",
                            constraint->joint_index, fixed->fixed_value, limit.first, limit.second);
            }
...
std::optional<std::pair<double, double>> JointConstrainedPlanningStrategy::getEffectiveConstraintRange(
    std::shared_ptr<JointConstraint> constraint,
    const std::vector<std::pair<double, double>>& joint_limits) const {

    if (!constraint || joint_limits.empty() ||
        constraint->joint_index < 0 ||
...
domain::entities::Trajectory JointConstrainedPlanningStrategy::plan(
    ...
    RCLCPP_INFO(rclcpp::get_logger("ConstrainedPlanningStrategy"),
                "Attempting to find trajectory satisfying constraints with %d attempts", max_attempts);
    ...
    for (int attempt = 0; attempt < max_attempts; ++attempt) {
        ...
        auto [trajectory, score] = attemptPlanWithSeed(goal_pose, planning_type, actual_seed_config);
        ...
        // 动态设置接受阈值
        double acceptance_threshold = has_soft_constraints ? 0.9 : 1.0; // 有软约束时降低要求

        if (score >= acceptance_threshold) {
            RCLCPP_INFO(rclcpp::get_logger("ConstrainedPlanningStrategy"),
                        "Found acceptable trajectory (score: %.3f) on attempt %d", score, attempt + 1);
    ...
    double final_threshold = has_soft_constraints ? 0.7 : 0.8; // 软约束时进一步降低要求

    if (found_valid && best_score > final_threshold) {
        RCLCPP_WARN(rclcpp::get_logger("ConstrainedPlanningStrategy"),
                    "No trajectory fully satisfies constraints. Using best solution with score %.3f (threshold: %.1f)",
                    best_score, final_threshold);
        return convertTrajectoryType(best_trajectory);
    }

    RCLCPP_ERROR(rclcpp::get_logger("ConstrainedPlanningStrategy"),
                 "Failed to find acceptable trajectory after %d attempts. Best score: %.3f",
...
domain::entities::Trajectory JointConstrainedPlanningStrategy::convertTrajectoryType(
    ...
    for (const auto& point : moveit_traj.joint_trajectory.points) {
        domain::entities::TrajectoryPoint traj_point{
            ...
            .acceleration = domain::value_objects::JointAcceleration(
                ...
                std::vector<double>(point.positions.size(), 0.0) : point.accelerations
            ),
...
double JointConstrainedPlanningStrategy::computeConstraintScore(
    ...
    for (const auto& point : trajectory.joint_trajectory.points) {
        for (const auto& constraint : constraints_) {
            if (constraint->joint_index >= static_cast<int>(point.positions.size())) {
                continue; // 跳过超出范围的约束
            }
            ...
            if (auto fixed = std::dynamic_pointer_cast<FixedJointConstraint>(constraint)) {
                double error = std::abs(point.positions[fixed->joint_index] - fixed->fixed_value);

                if (constraint->is_hard) {
                    // 硬约束：超出容差就算违反
                    if (error > DEFAULT_POSITION_TOLERANCE) {
                        violation = std::min(error / DEFAULT_POSITION_TOLERANCE, 10.0); // 最大10倍惩罚
                    }
                } else {
                    // 软约束：平方罚函数，距离越远惩罚越重
                    violation = (error * error) / (DEFAULT_POSITION_TOLERANCE * DEFAULT_POSITION_TOLERANCE);
                ...
                if (constraint->is_hard) {
                    ...
                    if (range_violation > DEFAULT_POSITION_TOLERANCE) {
                        violation = std::min(range_violation / DEFAULT_POSITION_TOLERANCE, 10.0);
                    }
                } else {
                    // 软约束：平方罚函数
                    if (range_violation > 0) {
                        violation = (range_violation * range_violation) / (DEFAULT_POSITION_TOLERANCE * DEFAULT_POSITION_TOLERANCE);
...
std::optional<std::vector<double>> JointConstrainedPlanningStrategy::generateSeedConfiguration(const std::string& arm_type, int attempt_index) const {
    ...
    for (size_t i = 0; i < joint_limits.size(); ++i) {
        ...
        if (joint_constraint) {
            // 获取约束的有效范围
            auto effective_range = getEffectiveConstraintRange(joint_constraint, joint_limits);
            if (effective_range.has_value()) {
                ...
                if (min_val == max_val) {
                    ...
                    // 范围约束
                    std::uniform_real_distribution<double> constrained_dis(min_val, max_val);
                    seed_config.push_back(constrained_dis(gen));
                }
            } else {
                // 约束无效，报错并返回
                RCLCPP_ERROR(rclcpp::get_logger("ConstrainedPlanningStrategy"),
                            "CONSTRAINT CONFLICT: Joint %zu constraint conflicts with joint limits. This will result in NO SOLUTION!",
                            i);
                return std::nullopt;
            }
        } else {
            // 没有约束，使用关节限制范围
            std::uniform_real_distribution<double> joint_dis(joint_limits[i].first, joint_limits[i].second);
...
std::pair<moveit_msgs::msg::RobotTrajectory, double> JointConstrainedPlanningStrategy::attemptPlanWithSeed(
    const geometry_msgs::msg::Pose& goal_pose,
    PlanningType planning_type,
