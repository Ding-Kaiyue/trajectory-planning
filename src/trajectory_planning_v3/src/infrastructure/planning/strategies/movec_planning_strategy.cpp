...
domain::entities::Trajectory MoveCPlanningStrategy::planArc(const geometry_msgs::msg::Pose& start_pose,
    ...
    if (std::fabs(d) < 1e-6)
    {
        ...
        for (int i = 0; i <= num_points; ++i)
        {
            ...
            // 线性插值位置
            p.position.x = start_pose.position.x + t * (goal_pose.position.x - start_pose.position.x);
            p.position.y = start_pose.position.y + t * (goal_pose.position.y - start_pose.position.y);
            p.position.z = start_pose.position.z + t * (goal_pose.position.z - start_pose.position.z);
    ...
    bool success = moveit_adapter_.planCartesianPath(
                       waypoints, moveit_traj);
...
domain::entities::Trajectory MoveCPlanningStrategy::planBezier(const geometry_msgs::msg::Pose& start,
    ...
    for (int i = 0; i <= num_points; ++i) {
        ...
        // 位置插值
        interpolated_pose.position.x = b0 * start.position.x + b1 * ctrl1.position.x +
                                       b2 * ctrl2.position.x + b3 * goal.position.x;
        interpolated_pose.position.y = b0 * start.position.y + b1 * ctrl1.position.y +
                                       b2 * ctrl2.position.y + b3 * goal.position.y;
        interpolated_pose.position.z = b0 * start.position.z + b1 * ctrl1.position.z +
                                       b2 * ctrl2.position.z + b3 * goal.position.z;

        // 姿态使用球面线性插值 (简化为线性插值)
        interpolated_pose.orientation.x = b0 * start.orientation.x + b1 * ctrl1.orientation.x +
                                          b2 * ctrl2.orientation.x + b3 * goal.orientation.x;
        interpolated_pose.orientation.y = b0 * start.orientation.y + b1 * ctrl1.orientation.y +
                                          b2 * ctrl2.orientation.y + b3 * goal.orientation.y;
        interpolated_pose.orientation.z = b0 * start.orientation.z + b1 * ctrl1.orientation.z +
                                          b2 * ctrl2.orientation.z + b3 * goal.orientation.z;
        interpolated_pose.orientation.w = b0 * start.orientation.w + b1 * ctrl1.orientation.w +
    ...
    // 使用MoveIt进行笛卡尔路径规划
    moveit_msgs::msg::RobotTrajectory moveit_traj;
    bool success = moveit_adapter_.planCartesianPath(bezier_waypoints, moveit_traj);
...
domain::entities::Trajectory MoveCPlanningStrategy::planCircle(const geometry_msgs::msg::Pose& center,
    ...
    double radius = std::sqrt(std::pow(radius_point.position.x - center.position.x, 2) +
    ...
    moveit_msgs::msg::RobotTrajectory moveit_traj;
    bool success = moveit_adapter_.planCartesianPath(circle_waypoints, moveit_traj);
...
domain::entities::Trajectory MoveCPlanningStrategy::planCircleThrough3Points(const geometry_msgs::msg::Pose& p1,
    ...
    center.position.z = (p1.position.z + p2.position.z + p3.position.z) / 3.0; // 取平均Z值
    ...
    for (int i = 0; i <= num_points; ++i) {
        ...
        waypoint.position.z = center.position.z + t * (p3.position.z - p1.position.z);

        // 线性插值姿态
        waypoint.orientation.x = (1-t) * p1.orientation.x + t * p3.orientation.x;
        waypoint.orientation.y = (1-t) * p1.orientation.y + t * p3.orientation.y;
        waypoint.orientation.z = (1-t) * p1.orientation.z + t * p3.orientation.z;
        waypoint.orientation.w = (1-t) * p1.orientation.w + t * p3.orientation.w;
