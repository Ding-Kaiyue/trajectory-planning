...
bool TrajectoryExecutor::execute(
	...
	if (trajectory.empty()) {
		RCLCPP_WARN(rclcpp::get_logger("TrajectoryExecutor"),
		            "Trajectory is empty!");
		return false;
	}
	...
	moveit_trajectory.joint_trajectory.joint_names =
	    moveit_adapter_.getJointNames();
...
void TrajectoryExecutor::executeWithProgress(
    const domain::entities::Trajectory& trajectory,
	...
	if (total_duration <= 0.0) {
		RCLCPP_WARN(
		    logger,
		    "Trajectory duration is zero or negative, cannot show progress");
	...
	RCLCPP_INFO(logger, "Executing trajectory (%.2f seconds)...",
	            total_duration);
