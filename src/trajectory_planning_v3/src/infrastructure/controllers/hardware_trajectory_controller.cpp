...
HardwareTrajectoryController::HardwareTrajectoryController(
    ...
    RCLCPP_INFO(this->get_logger(), "Hardware trajectory controller initialized");
...
HardwareTrajectoryController::HardwareTrajectoryController(
    ...
    RCLCPP_INFO(this->get_logger(), "Hardware trajectory controller initialized (observer mode)");
    RCLCPP_INFO(this->get_logger(), "Action server: %s", action_name.c_str());
}
...
rclcpp_action::GoalResponse HardwareTrajectoryController::handle_goal(
    ...
    if (trajectory_executing_) {
        RCLCPP_WARN(this->get_logger(), "Already executing trajectory, rejecting new goal");
        return rclcpp_action::GoalResponse::REJECT;
    }
...
void HardwareTrajectoryController::handle_accepted(
    ...
    // 在新线程中执行轨迹
    std::thread{std::bind(&HardwareTrajectoryController::execute_trajectory, this, _1), goal_handle}
        .detach();
}
...
void HardwareTrajectoryController::execute_trajectory(
    ...
    RCLCPP_INFO(this->get_logger(), "MoveIt give us %d trajectory points", point_num);
    ...
    if (point_num > 0) {
        ...
        double max_time = goal->trajectory.points[point_num - 1].time_from_start.sec + 
                         goal->trajectory.points[point_num - 1].time_from_start.nanosec / 1e9;
        RCLCPP_INFO(this->get_logger(), "Trajectory max_time is: %.3f seconds", max_time);

        // 打印ROS轨迹原始数据
        for (int i = 0; i < point_num; i++) {
            const auto& ros_point = goal->trajectory.points[i];
            double time_sec = ros_point.time_from_start.sec + ros_point.time_from_start.nanosec / 1e9;
            ...
            // RCLCPP_INFO(this->get_logger(), "ROS Point[%d] time=%.3fs:", i, time_sec);
            // RCLCPP_INFO(this->get_logger(), "  positions: [%.4f, %.4f, %.4f, %.4f, %.4f, %.4f]", 
            //            positions_deg[0], positions_deg[1], positions_deg[2],
            //            positions_deg[3], positions_deg[4], positions_deg[5]);
            // RCLCPP_INFO(this->get_logger(), "  velocities: [%.4f, %.4f, %.4f, %.4f, %.4f, %.4f]",
            //            ros_point.velocities[0], ros_point.velocities[1], ros_point.velocities[2],
            //            ros_point.velocities[3], ros_point.velocities[4], ros_point.velocities[5]);
            // RCLCPP_INFO(this->get_logger(), "  accelerations: [%.4f, %.4f, %.4f, %.4f, %.4f, %.4f]",
            //            ros_point.accelerations[0], ros_point.accelerations[1], ros_point.accelerations[2],
            //            ros_point.accelerations[3], ros_point.accelerations[4], ros_point.accelerations[5]);
            ...
            trajectory_planning::domain::entities::TrajectoryPoint domain_point{
                ...
                .time_from_start = trajectory_planning::domain::value_objects::Duration(time_sec),
...
void HardwareTrajectoryController::publishJointStates() {
...

    {
        ...
        for (size_t i = 0; i < joint_states_.size(); ++i) {
            joint_state_msg.position[i] = joint_states_[i].position * M_PI / 180.0;
