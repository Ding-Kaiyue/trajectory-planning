...
bool Constraint::is_within_limits(const std::vector<double>& positions,
                                  const std::vector<double>& velocities,
                                  const std::vector<double>& accelerations) const {
    size_t n = std::min({positions.size(), velocities.size(),
                         accelerations.size(), pos_limits_min_.size(),
                         pos_limits_max_.size(), vel_limits_.size(), acc_limits_.size()});

    for (size_t i = 0; i < n; ++i) {
        if (positions[i] < pos_limits_min_[i] || positions[i] > pos_limits_max_[i]) {
            return false;
        }
