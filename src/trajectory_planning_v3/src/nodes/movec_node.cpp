...
/* 
  ...
  ros2 topic pub /movec_goals_test   robot_interfaces/msg/MoveCTestRequest   "{route_type: 0,
    route_name: 'arc_test',
    waypoints: [
      {x: 0.4, y: 0.0, z: 0.4, roll: -120.0, pitch: -20.0, yaw: -90.0},
      {x: 0.35, y: 0.12, z: 0.45, roll: -50.0, pitch: 0.0, yaw: -90.0}
    ]}" -1
  */
...
public:
    MoveCNode() : Node("movec_node"), tf_buffer_(this->get_clock()), tf_listener_(tf_buffer_) {
        // ---- 1. 声明参数 ----
        ...
        goal_sub_ = this->create_subscription<robot_interfaces::msg::MoveCRequest>(
            "/movec_goals", qos,
            std::bind(&MoveCNode::goalCallback, this, std::placeholders::_1)
        );

        // 订阅更灵活的RPY测试话题
        goal_test_sub_ = this->create_subscription<robot_interfaces::msg::MoveCTestRequest>(
            "/movec_goals_test", qos,
            std::bind(&MoveCNode::goalTestCallback, this, std::placeholders::_1)
        );

        RCLCPP_INFO(this->get_logger(), "MoveC Node ready. Publish to:\n"
    ...
    void initializeComponents() {
        if (!motion_planning_service_) {
            ...
            motion_planning_service_ = std::make_shared<MotionPlanningService>(
                nullptr, nullptr, movec_strategy, nullptr,
            ...
            RCLCPP_INFO(this->get_logger(), "MoveItAdapter, MoveCPlanningStrategy and TrajectoryExecutor initialized.");
        }
    }

private:
    void goalCallback(const robot_interfaces::msg::MoveCRequest::SharedPtr msg) {
    ...
    void goalTestCallback(const robot_interfaces::msg::MoveCTestRequest::SharedPtr msg) {
        RCLCPP_INFO(this->get_logger(), "Received test request: route_type=%d, route_name='%s', waypoints=%zu",
                   msg->route_type, msg->route_name.c_str(), msg->waypoints.size());

        if (msg->waypoints.empty()) {
            RCLCPP_ERROR(this->get_logger(), "No waypoints received in test request!");
            return;
        }

        // 转换为标准的MoveCRequest消息
        auto movec_msg = std::make_shared<robot_interfaces::msg::MoveCRequest>();
    ...
    // ROS订阅者
    rclcpp::Subscription<robot_interfaces::msg::MoveCRequest>::SharedPtr goal_sub_;
    rclcpp::Subscription<robot_interfaces::msg::MoveCTestRequest>::SharedPtr goal_test_sub_;
