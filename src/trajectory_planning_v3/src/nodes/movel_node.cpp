...
// ros2 topic pub --once /movel_goals_rpy geometry_msgs/msg/Vector3 "{x: -80.0,
// y: 0.0, z: -90.0}"
...
public:
	MoveLNode() : Node("movel_node") {
		...
		goal_rpy_sub_ = this->create_subscription<geometry_msgs::msg::Vector3>(
		    "/movel_goals_rpy", qos,
		    std::bind(&MoveLNode::goalRPYCallback, this,
		              std::placeholders::_1));

		RCLCPP_INFO(this->get_logger(),
		            "MoveL Node ready. Publish to:\n"
	...
	void initializeComponents() {
		if (!motion_planning_service_) {
			...
			RCLCPP_INFO(this->get_logger(),
			            "MoveItAdapter, MoveLPlanningStrategy and "
...
private:
	...
	void goalRPYCallback(const geometry_msgs::msg::Vector3::SharedPtr msg) {
		...
		// 将RPY转换为四元数
		tf2::Quaternion q;
		q.setRPY(msg->x * M_PI / 180.0, msg->y * M_PI / 180.0,
		         msg->z * M_PI / 180.0);
		...
		RCLCPP_INFO(this->get_logger(),
		            "Received RPY goal: roll=%.1f°, pitch=%.1f°, yaw=%.1f°",
		            msg->x, msg->y, msg->z);
