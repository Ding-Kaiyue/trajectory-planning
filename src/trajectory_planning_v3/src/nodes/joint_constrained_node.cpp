...
/*
    ros2 topic pub --once /joint_constrained_goals_test
   robot_interfaces/msg/JointConstrainedTestRequest \
    "{
    ...
    joint_constraints: [
        ...
        {type: 1, joint_index: 3, weight: 0.8, is_hard: false, min_value: -0.5,
   max_value: 0.5}
    ]
    }"

    ros2 topic pub --once /joint_constrained_goals_test
   robot_interfaces/msg/JointConstrainedTestRequest "{ goal_position: {x: 0.4,
   y: 0.0, z: 0.4}, roll: -90.0, pitch: 0.0, yaw: -90.0, planning_type: 2,
        max_attempts: 5,
        joint_constraints: [
            {type: 0, joint_index: 0, weight: 1.0, is_hard: true, fixed_value:
   0.0}, {type: 1, joint_index: 3, weight: 0.8, is_hard: false, min_value: -0.2,
   max_value: 0.2}
        ]
    }"
 */
/**
 * @brief
 * 关节约束运动节点，订阅JointConstrainedMotion消息，规划并执行关节约束运动
...
public:
	JointConstrainedNode() : Node("joint_constrained_node") {
		...
		// ---- 2. 订阅 /joint_constrained_goals 和
		// /joint_constrained_goals_test ----
		rclcpp::QoS qos(10);
		qos.reliability(rclcpp::ReliabilityPolicy::Reliable);
		goal_sub_ = this->create_subscription<
		    robot_interfaces::msg::JointConstrainedRequest>(
		    "/joint_constrained_goals", qos,
		    std::bind(&JointConstrainedNode::goalCallback, this,
		              std::placeholders::_1));
		goal_test_sub_ = this->create_subscription<
		    robot_interfaces::msg::JointConstrainedTestRequest>(
		    "/joint_constrained_goals_test", qos,
		    std::bind(&JointConstrainedNode::goalTestCallback, this,
		              std::placeholders::_1));
		RCLCPP_INFO(
		    this->get_logger(),
		    "Joint Constrained Node ready. Publish to: \n"
		    "  /joint_constrained_goals(JointConstrainedRequest) - Full "
		    "message\n"
		    "  /joint_constrained_goals_test(JointConstrainedTestRequest) - "
		    "RPY test message\n");
	}

	void initializeComponents() {
		if (!motion_planning_service_) {
			// 创建 MoveItAdapter, JointConstrainedPlanningStrategy 和
			// TrajectoryExecutor
			...
			auto joint_constrained_strategy_ =
			    std::make_shared<JointConstrainedPlanningStrategy>(
			        *moveit_adapter_);
			...
			// 注册需要的策略
			motion_planning_service_->registerJointConstrainedStrategy(
			...
			RCLCPP_INFO(this->get_logger(),
			            "MoveItAdapter, JointConstrainedPlanningStrategy and "
...
private:
	void goalCallback(
	    const robot_interfaces::msg::JointConstrainedRequest::SharedPtr msg) {
		...
		if (result.success) {
			...
			RCLCPP_ERROR(this->get_logger(), "约束规划失败：%s",
			             result.error_message.c_str());
		}
	}

	void goalTestCallback(
	    const robot_interfaces::msg::JointConstrainedTestRequest::SharedPtr
	        msg) {
		// 转换 TestRequest 到 Request 消息格式
		auto request_msg =
		    std::make_shared<robot_interfaces::msg::JointConstrainedRequest>();
		...
		tf2::Quaternion quat;
		quat.setRPY(msg->roll * M_PI / 180.0, msg->pitch * M_PI / 180.0,
		            msg->yaw * M_PI / 180.0);
	...
	// ROS订阅者
	rclcpp::Subscription<
	    robot_interfaces::msg::JointConstrainedRequest>::SharedPtr goal_sub_;
	rclcpp::Subscription<robot_interfaces::msg::JointConstrainedTestRequest>::
	    SharedPtr goal_test_sub_;
};
