...
int main(int argc, char* argv[]) {
	...
	try {
		...
		// 先创建硬件轨迹控制器（作为观察者）
		auto controller =
		    std::make_shared<trajectory_planning::infrastructure::controllers::
		                         HardwareTrajectoryController>(
		        "can0", "arm_controller");

		// 创建RobotHardware，将controller作为观察者传入
		auto robot_hw = std::make_shared<RobotHardware>(
		    motor_driver, motor_config, controller);

		// 创建硬件适配器，直接传入RobotHardware
		auto hardware_adapter = std::make_shared<
		    trajectory_planning::infrastructure::adapters::HardwareAdapter>(
		    robot_hw, "can0", 6);
