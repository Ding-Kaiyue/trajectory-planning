...
ExecutionResult TrajectoryExecutionService::execute(const Trajectory& trajectory) {
    ...
    try {
        ...
        return createFailureResult(std::string("执行过程中发生异常：") + e.what());
    }
}
...
void TrajectoryExecutionService::stop() {
    ...
    try {
        ...
        RCLCPP_WARN(logger_, "停止功能暂未实现 - 当前 TrajectoryExecutor 不支持停止操作");
...
ExecutionResult TrajectoryExecutionService::createFailureResult(const std::string& reason) {
    RCLCPP_ERROR(logger_, "轨迹执行失败：%s", reason.c_str());
    return ExecutionResult(false, reason);
}

ExecutionResult TrajectoryExecutionService::createSuccessResult(const std::string& message) {
    if (!message.empty()) {
