...
void MotionPlanningService::registerMoveCStrategy(
    std::shared_ptr<MoveCPlanningStrategy> strategy) {
...
PlanningResult MotionPlanningService::planJointMotion(
    const sensor_msgs::msg::JointState& goal) {
	if (!movej_strategy_) {
		...
		switch (request.route_type) {
			...
			default:
				return createFailureResult(
				    "MoveC", "Unsupported route_type: " +
				                 std::to_string(request.route_type));
		}
...
Trajectory MotionPlanningService::planCircleTrajectory(
	...
	if (request.waypoints.size() != 2) {
		RCLCPP_ERROR(
		    logger_,
		    "CIRCLE route requires exactly 2 waypoints: [start, center]");
		return Trajectory{};
	}
...
Trajectory MotionPlanningService::planCircle3PtTrajectory(
	...
	if (request.waypoints.size() != 3) {
		RCLCPP_ERROR(
		    logger_,
		    "CIRCLE3PT route requires exactly 3 waypoints: [p1, p2, p3]");
		return Trajectory{};
	}

	return movec_strategy_->planCircleThrough3Points(
	    request.waypoints[0], request.waypoints[1], request.waypoints[2]);
}

PlanningResult MotionPlanningService::createFailureResult(
