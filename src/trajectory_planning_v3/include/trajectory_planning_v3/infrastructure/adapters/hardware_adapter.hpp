...
public:
    /**
     * @brief 构造函数
     * @param robot_hw RobotHardware实例（可以为nullptr，稍后通过setRobotHardware设置）
     * @param interface 硬件接口名称（如"can0"）
     * @param num_joints 关节数量
     */
    explicit HardwareAdapter(
...
private:
     ...
     * @return 转换后的轨迹
     */
    ::Trajectory convertTrajectory(const domain::entities::Trajectory& traj) const;
