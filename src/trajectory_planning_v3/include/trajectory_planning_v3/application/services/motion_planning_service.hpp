...
private:
    ...
    // 辅助方法
    Trajectory planArcTrajectory(const robot_interfaces::msg::MoveCRequest& request);
    Trajectory planBezierTrajectory(const robot_interfaces::msg::MoveCRequest& request);
    Trajectory planCircleTrajectory(const robot_interfaces::msg::MoveCRequest& request);
    Trajectory planCircle3PtTrajectory(const robot_interfaces::msg::MoveCRequest& request);
    PlanningResult createFailureResult(const std::string& operation, const std::string& reason);
    PlanningResult createSuccessResult(const Trajectory& trajectory, const std::string& operation);
};
